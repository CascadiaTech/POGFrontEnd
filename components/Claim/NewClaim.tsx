...
export default function NewClaim() {
  ...
  function FetchReserves() {
    ...
    try {
      setLoading(true);
      const reservesArray = Array.isArray(getReserves)
        ? (getReserves as bigint[])
        : [];
      const convertedNumbers = reservesArray.map((bn: any) => Number(bn));
      console.log(convertedNumbers, "array of javascript numbers");
      const reserveToken0 = convertedNumbers[0];
      const reserveToken1 = convertedNumbers[1];
      const lpPrice = reserveToken0 / reserveToken1;
      const divPrice = lpPrice;

      console.log("LP Price:", divPrice);
      if (Number.isNaN(lpPrice)) {
        return 0;
      }
